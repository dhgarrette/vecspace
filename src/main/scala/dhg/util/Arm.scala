...
object Arm {
  ...
  class ManagedCloseable[T <: Closeable](val self: T) extends Managed[T] {
    def close() { self.close() }
  }
  implicit def toManagedCloseable[T <: Closeable](self: T) = new ManagedCloseable(self)

  class ManagedSource[T <: Source](val self: T) extends Managed[T] {
    def close() { self.close() }
  }
  implicit def toManagedSource[T <: Source](self: T) = new ManagedSource(self)
