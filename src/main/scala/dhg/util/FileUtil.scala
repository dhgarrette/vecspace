...
object FileUtil {
  ...
  implicit def toEnhancedFile(self: File) = new EnhancedFile(self)
  class EnhancedFile(val self: File) {
    ...
    def ls() = {
      assert(self.exists, "'%s' does not exist".format(self))
      assert(self.isDirectory, "'%s' is not a directory".format(self))
      self.listFiles
    }
  ...
  def findBinary(name: String, binDir: Option[String] = None, envar: Option[String] = None): String = {
    ...
    try {
      ...
      case _: Throwable => checked += "which " + name
    }
   ...
   */
  implicit def toWriterWithWriteLine(self: Writer) = new WriterWithWriteLine(self)
  class WriterWithWriteLine(val self: Writer) {
